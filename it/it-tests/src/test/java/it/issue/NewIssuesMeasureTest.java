...
public class NewIssuesMeasureTest extends AbstractIssueTest {
  ...
  public void new_issues_measures() throws Exception {
    ...
    Map<Integer, String> newIssues = getPeriodMeasureValuesByIndex(ORCHESTRATOR, "sample:src/main/xoo/sample/Sample.xoo", "new_violations");
    assertThat(parseInt(newIssues.get(1))).isEqualTo(17);
    assertThat(parseInt(newIssues.get(2))).isEqualTo(17);

    // second analysis, with exactly the same profile -> no new issues
    ORCHESTRATOR.executeBuild(SonarScanner.create(projectDir("shared/xoo-sample")));

    assertThat(ORCHESTRATOR.getServer().wsClient().issueClient().find(IssueQuery.create()).list()).isNotEmpty();
    assertThat(getPeriodMeasureValuesByIndex(ORCHESTRATOR, "sample:src/main/xoo/sample/Sample.xoo", "new_violations").values())
      .extracting(Integer::parseInt).containsOnly(0, 0, 0);
  }
  ...
  public void new_issues_measures_should_be_zero_on_project_when_no_new_issues_since_x_days() throws Exception {
    ...
    Map<Integer, String> measures = getPeriodMeasureValuesByIndex(ORCHESTRATOR, "sample", "new_violations");
    assertThat(parseInt(measures.get(1))).isZero();
    assertThat(parseInt(measures.get(2))).isZero();
  }
  ...
  public void new_issues_measures_should_be_correctly_calculated_when_adding_a_new_module() throws Exception {
    ...
    Map<Integer, String> periodMeasures = getPeriodMeasureValuesByIndex(ORCHESTRATOR, "com.sonarsource.it.samples:multi-modules-sample", "new_violations");
    assertThat(parseInt(periodMeasures.get(1))).isEqualTo(65);
  }
